...
var connString = "postgres://postgres:bob@localhost:5432/assessbox";
...
app.post('/api/vehicles', function(req, res) {
    db.create_vehicle([
        req.body.make, req.body.model, req.body.year, req.body.ownerId
    ], function(err, success) {
        res.status(200).json(success);
    })
});
...
app.get('/api/vehicle', function(req, res) {
    ...
    if (req.query.userFirstStart) {
        db.get_vehicles_by_firstStart([req.query.userFirstStart], function(err, success) {
            res.status(200).json(success);
        })
    }
...
app.put('/api/vehicle/:vehicleId/user/:userId', function(req, res) {
    db.change_vehicle_owner([
        req.params.vehicleId, req.params.userId
    ], function(err, success) {
        res.status(200).json(success);
    })
});
